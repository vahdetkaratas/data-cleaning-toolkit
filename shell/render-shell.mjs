...
async function main() {
  ...
  const logo1 = project.logoLine1 || "Project";
  const logo2 = project.logoLine2 || "Overview";
  const heroTitle = project.heroTitle || project.title;
  const footerTitle = project.footerProjectName || "Portfolio project";
  const footerSub =
    project.footerProjectSub ||
    "Static shell showcase";
  ...
  const rendered = template
    ...
    .replaceAll(
      "{{PROJECT_EYEBROW}}",
      escapeHtml(project.eyebrow || "Project overview")
    )
    ...
    .replaceAll(
      ...
      escapeHtml(
        profile.footerTagline ||
          "Data systems and preprocessing tooling."
      )
    );
